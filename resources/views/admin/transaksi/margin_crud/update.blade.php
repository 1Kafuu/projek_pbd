            ...
            <div class="bg-white border border-gray-300 rounded-lg shadow-sm p-6">
                <div
                    ...
                    <div class="mb-4 sm:mb-0">
                        <h3 class="text-2xl font-semibold text-gray-800">Update Margin</h3>
                    </div>
                </div>

                <!-- Form -->
                <form action="{{ route('update-margin', ['id' => $result->idmargin_penjualan]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">

                        <div class="sm:col-span-6">
                            <label for="persen" class="block text-sm font-medium text-gray-700 mb-1">Persen</label>
                            <div class="relative">
                                <input id="persen" type="decimal" name="persen" placeholder="Masukkan Persen..." value="{{ $result->persen }}"
                                    class="block w-full rounded-md border border-gray-300 bg-white py-2 px-3 text-base text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('persen') border-red-500  @enderror"
                                    required>
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="relative">
                                <select id="status" name="status"
                                    ...
                                    <option value="2" {{ $result->status == '0' ? 'selected' : '' }}>Inactive</option>
                                </select>
                                @error('satuan')
