...
@section('content')
    {{-- {{ dd($result) }} --}}
                        ...
                        <div
                            ...
                            <div>
                                ...
                                <div class="flex justify-end gap-2 mb-4">
                                    <a href="{{ route('margin', ['filter' => 'active']) }}"
                                    ...
                                    <a href="{{ route('margin', ['filter' => 'all']) }}"
                                        class="px-3 py-1.5 rounded-md text-sm font-medium transition-colors duration-200
        ...
        $status_margin = $margin->STATUS_MARGIN
                                            @endphp
                                            <tr class="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                     <div class="flex items-center">
                                                        <?php
        $status = trim(strtolower(htmlspecialchars($status_margin)));
        $bgColor = $status === 'active' ? 'bg-green-100 text-green-800' :
            ($status === 'inactive' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800');
        $borderColor = $status === 'active' ? 'border-green-400' :
            ($status === 'inactive' ? 'border-red-400' : 'border-gray-400');
                                                        ?>

                                                        <span
                                                            class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border {{ $bgColor }} {{ $borderColor }}">
                                                            {{ htmlspecialchars($status_margin) }}
                                                        </span>
                                                    </div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-2">
                                                    <a href="{{ route('getIDMargin', ['id' => $margin->NO_MARGIN]) }}"
                                                    ...
                                                    <form action="{{ route('delete-margin', ['id' => $margin->NO_MARGIN]) }}" method="POST"
                                                        class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
