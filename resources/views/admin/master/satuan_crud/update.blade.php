    ...
    <div class="min-h-screen bg-gray-50">
        <div class="container mx-auto max-w-2xl px-4 py-8">
            ...
            <div class="bg-white border border-gray-300 rounded-lg shadow-sm p-6">
                ...
                <!-- Form -->
                <form action="{{ route('update-satuan', ['id' => $data->NO_SATUAN]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                        <div class="sm:col-span-6">
                            <label for="satuan" class="block text-sm font-medium text-gray-700 mb-1">Nama Vendor</label>
                            <div class="relative">
                                <input id="satuan" type="text" name="satuan" placeholder="Masukkan Nama Vendor..." value="{{ $data->NAMA_SATUAN }}"
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="relative">
                                <select id="status" name="status"
                                    ...
                                    <option value="1" {{ $data->STATUS_SATUAN == 'Active' ? 'selected' : '' }}>Active</option>
                                    <option value="2" {{ $data->STATUS_SATUAN == 'Inactive' ? 'selected' : '' }}>Inactive</option>
                                </select>
                                @error('status')
