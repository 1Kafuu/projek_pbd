                    ...
                    <div
                        ...
                        <div>
                            ...
                            <div class="flex justify-end gap-2 mb-4">
                                <a href="{{ route('datavendor', ['filter' => 'active']) }}"
                                ...
                                <a href="{{ route('datavendor', ['filter' => 'all']) }}" class="px-3 py-1.5 rounded-md text-sm font-medium transition-colors duration-200
                                        ...
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <span
                                                        class="text-sm text-gray-700">{{ htmlspecialchars($vendor->NAMA_VENDOR) }}</span>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <?php
                                                        $status = trim(strtolower(htmlspecialchars($vendor->BADAN_HUKUM)));
                                                        $bgColor = $status === 'yes' ? 'bg-green-100 text-green-800' :
                                                            ($status === 'no' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800');
                                                        $borderColor = $status === 'yes' ? 'border-green-400' :
                                                            ($status === 'no' ? 'border-red-400' : 'border-gray-400');
                                                    ?>

                                                    <span
                                                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border {{ $bgColor }} {{ $borderColor }}">
                                                        {{ htmlspecialchars($vendor->BADAN_HUKUM) }}
                                                    </span>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    @php
                                                        $status = trim(strtolower(htmlspecialchars($vendor->STATUS_VENDOR)));
                                                    ...
                                                    <span
                                                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border {{ $bgColor }} {{ $borderColor }}">
                                                        {{ htmlspecialchars($vendor->STATUS_VENDOR) }}
                                                    </span>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-2">
                                                <a href="{{ route('getIDVendor', ['id' => $vendor->NO_VENDOR]) }}"
                                                ...
                                                <form action="{{ route('delete-vendor', ['id' => $vendor->NO_VENDOR]) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
